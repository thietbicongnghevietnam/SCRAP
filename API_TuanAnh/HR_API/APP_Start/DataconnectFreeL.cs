...
namespace HR_API.APP_Start
{
    ...
    public class DataconnectFreeL 
    {
        ...
        public static string source = "Data Source=192.168.128.130;Initial Catalog=SWMS_DEV;User ID=swms_write;Password=sWms2023@;MultipleActiveResultSets=True;TrustServerCertificate=True;";
        private static SqlConnection con = new SqlConnection(DataconnectFreeL.source);

        static DataconnectFreeL()
        {
            try
            {
                DataconnectFreeL.con.Open();
            }
            catch
            {
            }
        ...
        {
            DataconnectFreeL.open();
            DataconnectFreeL.objCommand = new SqlCommand(storedname, DataconnectFreeL.objCnn);
            DataconnectFreeL.objCommand.CommandTimeout = 50;
            DataconnectFreeL.objCommand.CommandType = CommandType.StoredProcedure;
            DataTable dataTable = new DataTable();
            new SqlDataAdapter()
            {
                SelectCommand = DataconnectFreeL.objCommand
            }.Fill(dataTable);
            DataconnectFreeL.close();
            return dataTable;
        }
        ...
        {
            DataconnectFreeL.objCnn = new SqlConnection(DataconnectFreeL.GetConnectStringFromFile() + "User ID=scan;Password=khong123;");
            if (DataconnectFreeL.objCnn.State == ConnectionState.Open)
                return;
            DataconnectFreeL.objCnn.Open();
        }
        ...
        public static void close()
        {
            try
            {
                if (DataconnectFreeL.objCnn.State == ConnectionState.Closed)
                    return;
                DataconnectFreeL.objCnn.Close();
            }
            catch
            {
            }
        ...
        {
            SqlConnection connection = new SqlConnection(DataconnectFreeL.source);
            int num;
            try
            {
                connection.Open();
                DataconnectFreeL.objCommand = new SqlCommand(StoreName, connection);
                DataconnectFreeL.objCommand.CommandType = CommandType.StoredProcedure;
                for (int index = 0; index < ParameterList.Length; ++index)
                    DataconnectFreeL.objCommand.Parameters.Add(new SqlParameter(ParameterList[index], objValue[index]));
                num = DataconnectFreeL.objCommand.ExecuteNonQuery();
                connection.Dispose();
                connection.Close();
            }
            catch
          ...
          object[] objVal)
        {
            ...
            try
            {
                ...
                for (int index = 0; index < parameter.Length; ++index)
                    sqlCommand2.Parameters.Add(new SqlParameter(parameter[index], objVal[index]));
                DataconnectFreeL.da = new SqlDataAdapter();
                DataconnectFreeL.da.SelectCommand = sqlCommand2;
                DataconnectFreeL.da.Fill(dataSet2);
                connection.Dispose();
                connection.Close();
                return dataSet2;
            }
