...
namespace HR_API.APP_Start
{
    ...
    public class DataconnectSub
    {
        ...
        public static string source = "Data Source=192.168.128.1;Initial Catalog=SUB_CONTROL;User ID=sa;Password=Psnvdb2013;MultipleActiveResultSets=True;TrustServerCertificate=True;";
        private static SqlConnection con = new SqlConnection(DataconnectSub.source);

        static DataconnectSub()
        {
            try
            {
                DataconnectSub.con.Open();
            }
            catch
            {
            }
        ...
        {
            DataconnectSub.open();
            DataconnectSub.objCommand = new SqlCommand(storedname, DataconnectSub.objCnn);
            DataconnectSub.objCommand.CommandTimeout = 50;
            DataconnectSub.objCommand.CommandType = CommandType.StoredProcedure;
            DataTable dataTable = new DataTable();
            new SqlDataAdapter()
            {
                SelectCommand = DataconnectSub.objCommand
            }.Fill(dataTable);
            DataconnectSub.close();
            return dataTable;
        }
        ...
        {
            DataconnectSub.objCnn = new SqlConnection(DataconnectSub.GetConnectStringFromFile() + "User ID=scan;Password=khong123;");
            if (DataconnectSub.objCnn.State == ConnectionState.Open)
                return;
            DataconnectSub.objCnn.Open();
        }
        ...
        public static void close()
        {
            try
            {
                if (DataconnectSub.objCnn.State == ConnectionState.Closed)
                    return;
                DataconnectSub.objCnn.Close();
            }
            catch
            {
            }
        ...
        {
            SqlConnection connection = new SqlConnection(DataconnectSub.source);
            int num;
            try
            {
                connection.Open();
                DataconnectSub.objCommand = new SqlCommand(StoreName, connection);
                DataconnectSub.objCommand.CommandType = CommandType.StoredProcedure;
                for (int index = 0; index < ParameterList.Length; ++index)
                    DataconnectSub.objCommand.Parameters.Add(new SqlParameter(ParameterList[index], objValue[index]));
                num = DataconnectSub.objCommand.ExecuteNonQuery();
                connection.Dispose();
                connection.Close();
            }
            catch
          ...
          object[] objVal)
        {
            ...
            try
            {
                ...
                for (int index = 0; index < parameter.Length; ++index)
                    sqlCommand2.Parameters.Add(new SqlParameter(parameter[index], objVal[index]));
                DataconnectSub.da = new SqlDataAdapter();
                DataconnectSub.da.SelectCommand = sqlCommand2;
                DataconnectSub.da.Fill(dataSet2);
                connection.Dispose();
                connection.Close();
                return dataSet2;
            }
