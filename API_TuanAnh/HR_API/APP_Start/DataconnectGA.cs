...
namespace HR_API.APP_Start
{
    public class DataconnectGA
    {
        ...
        public static string source = "Data Source=192.168.128.1;Initial Catalog=Warehouse_BPS;User ID=sa;Password=Psnvdb2013;MultipleActiveResultSets=True;TrustServerCertificate=True;";
        private static SqlConnection con = new SqlConnection(DataconnectGA.source);

        static DataconnectGA()
        {
            try
            {
                DataconnectGA.con.Open();
            }
            catch
            {
            }
        ...
        {
            DataconnectGA.open();
            DataconnectGA.objCommand = new SqlCommand(storedname, DataconnectGA.objCnn);
            DataconnectGA.objCommand.CommandTimeout = 50;
            DataconnectGA.objCommand.CommandType = CommandType.StoredProcedure;
            DataTable dataTable = new DataTable();
            new SqlDataAdapter()
            {
                SelectCommand = DataconnectGA.objCommand
            }.Fill(dataTable);
            DataconnectGA.close();
            return dataTable;
        }
        ...
        {
            DataconnectGA.objCnn = new SqlConnection(DataconnectGA.GetConnectStringFromFile() + "User ID=scan;Password=khong123;");
            if (DataconnectGA.objCnn.State == ConnectionState.Open)
                return;
            DataconnectGA.objCnn.Open();
        }
        ...
        public static void close()
        {
            try
            {
                if (DataconnectGA.objCnn.State == ConnectionState.Closed)
                    return;
                DataconnectGA.objCnn.Close();
            }
            catch
            {
            }
        ...
        {
            SqlConnection connection = new SqlConnection(DataconnectGA.source);
            int num;
            try
            {
                connection.Open();
                DataconnectGA.objCommand = new SqlCommand(StoreName, connection);
                DataconnectGA.objCommand.CommandType = CommandType.StoredProcedure;
                for (int index = 0; index < ParameterList.Length; ++index)
                    DataconnectGA.objCommand.Parameters.Add(new SqlParameter(ParameterList[index], objValue[index]));
                num = DataconnectGA.objCommand.ExecuteNonQuery();
                connection.Dispose();
                connection.Close();
            }
            catch
          ...
          object[] objVal)
        {
            ...
            try
            {
                ...
                for (int index = 0; index < parameter.Length; ++index)
                    sqlCommand2.Parameters.Add(new SqlParameter(parameter[index], objVal[index]));
                DataconnectGA.da = new SqlDataAdapter();
                DataconnectGA.da.SelectCommand = sqlCommand2;
                DataconnectGA.da.Fill(dataSet2);
                connection.Dispose();
                connection.Close();
                return dataSet2;
            }
