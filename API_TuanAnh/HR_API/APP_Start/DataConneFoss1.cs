...
namespace HR_API.APP_Start
{
    public class DataConneFoss1
    {
        ...
        public static string source = "Data Source=192.168.128.1;Initial Catalog=FOSS1;User ID=sa;Password=Psnvdb2013;MultipleActiveResultSets=True;TrustServerCertificate=True;";
        private static SqlConnection con = new SqlConnection(DataConneFoss1.source);

        static DataConneFoss1()
        {
            try
            {
                DataConneFoss1.con.Open();
            }
            catch
            {
            }
        ...
        {
            DataConneFoss1.open();
            DataConneFoss1.objCommand = new SqlCommand(storedname, DataConneFoss1.objCnn);
            DataConneFoss1.objCommand.CommandTimeout = 50;
            DataConneFoss1.objCommand.CommandType = CommandType.StoredProcedure;
            DataTable dataTable = new DataTable();
            new SqlDataAdapter()
            {
                SelectCommand = DataConneFoss1.objCommand
            }.Fill(dataTable);
            DataConneFoss1.close();
            return dataTable;
        }
        ...
        {
            DataConneFoss1.objCnn = new SqlConnection(DataConneFoss1.GetConnectStringFromFile() + "User ID=scan;Password=khong123;");
            if (DataConneFoss1.objCnn.State == ConnectionState.Open)
                return;
            DataConneFoss1.objCnn.Open();
        }
        ...
        public static void close()
        {
            try
            {
                if (DataConneFoss1.objCnn.State == ConnectionState.Closed)
                    return;
                DataConneFoss1.objCnn.Close();
            }
            catch
            {
            }
        ...
        {
            SqlConnection connection = new SqlConnection(DataConneFoss1.source);
            int num;
            try
            {
                connection.Open();
                DataConneFoss1.objCommand = new SqlCommand(StoreName, connection);
                DataConneFoss1.objCommand.CommandType = CommandType.StoredProcedure;
                for (int index = 0; index < ParameterList.Length; ++index)
                    DataConneFoss1.objCommand.Parameters.Add(new SqlParameter(ParameterList[index], objValue[index]));
                num = DataConneFoss1.objCommand.ExecuteNonQuery();
                connection.Dispose();
                connection.Close();
            }
            catch
          ...
          object[] objVal)
        {
            ...
            try
            {
                ...
                for (int index = 0; index < parameter.Length; ++index)
                    sqlCommand2.Parameters.Add(new SqlParameter(parameter[index], objVal[index]));
                DataConneFoss1.da = new SqlDataAdapter();
                DataConneFoss1.da.SelectCommand = sqlCommand2;
                DataConneFoss1.da.Fill(dataSet2);
                connection.Dispose();
                connection.Close();
                return dataSet2;
            }
